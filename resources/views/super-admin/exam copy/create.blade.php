				...
				<div class="card">
					<div class="card-header">
						<div class="row mb-3">
							<div class="col-12 text-center">
								...
								{{-- <h4>{{ $question->exam_name ?? null }}</h4> --}}
								{{-- <h5 class="m-0">Start Exam: {{ $question->exam_date ?? null }}</h5> --}}
							</div>
						</div>
					...
					<div class="card-body">
						...
						@foreach ($questions as $key => $question)
							<div class="col-12 mb-5" id="questions">
								...
								@foreach ($questionOptions as $optionKey => $option)
									<div class="border rounded p-3 my-2">
										<div class="form-check">
											<input id="option_{{ $option->id }}" class="form-check-input font-size-14" type="radio" name="answer[]" value="{{ $option->option }}">
						...
						<div class="card-footer">
							<div class="row align-items-center">
								...
								<div class="col-md-4 text-end">
									...
									<button type="button" class="btn btn-primary waves-effect waves-light w-md" id="submit"><i class="fas fa-save me-2"></i>Submit Answer</button>
								</div>
							</div>
...
@push('js')
	<script>
		$(document).ready(function() {
			// Timer logic
			var totalSeconds = {{ $questions->count() * 2 }} * 60;
			var timer = setInterval(function() {
				...
				if (totalSeconds <= 0) {
					clearInterval(timer);
					// Optionally, you can submit the quiz form or perform other actions when time is up
					console.log('Time is up!');
					// Example: $('#quizForm').submit();
				}
			}, 1000);
