								...
								<div class="row">
									...
									<div class="col-md-6">
										<div class="form-group mb-3">
											...
											<select name="subject_id" class="form-control form-select @error('subject_id') is-invalid @enderror" id="search_id">
									...
									{{-- <div class="col-md-6">
										<div class="form-group mb-3">
											...
											<select name="question_type" class="form-control form-select question_type @error('question_type') is-invalid @enderror">
	...
	<script>
		$(document).ready(function() {
			$('#department_id').on('change', function() {
				...
				// Clear existing options in search_id dropdown
				$('#search_id').empty().append('<option disabled selected>-- Loading Subjects --</option>');
				...
				$.ajax({
					...
					success: function(response) {
						if (response.length > 0) {
							// Append options to search_id dropdown
							$('#search_id').empty().append('<option disabled selected>-- Select Subject --</option>');
							$.each(response, function(key, value) {
								$('#search_id').append('<option value="' + value.id + '">' + value.subject_name + '</option>');
							});
						} else {
							$('#search_id').empty().append('<option disabled selected>-- Subject Not Found --</option>');
						}
					},
				});
